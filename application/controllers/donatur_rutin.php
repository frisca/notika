...
class Donatur_Rutin extends CI_Controller {
	...
	{
		$data['donatur'] = $this->all_model->getListDonaturRutin()->result();
		$this->load->view('donatur-rutin/index', $data);
	}

	public function add()
	{
		$this->load->view('donatur-rutin/add');
	}
	...
	public function processAdd()
	{
		...
		if($result_user == true){
			...
			if($result_donatur == true){
				return redirect(base_url() . 'donatur_rutin/index');
			}
		}
		return redirect(base_url() . 'donatur_rutin/add');
	}

	public function edit($id)
	{
		$data['donatur'] = $this->all_model->getDonaturRutinById((int) $id)->row();
		$this->load->view('donatur-rutin/edit', $data);
	}

	public function processEdit()
	{
		$donatur = $this->all_model->getDonaturRutinById((int) $this->input->post('idDonatur'))->row();
		$user = array(
			'nama' => $this->input->post('nama'),
			'alamat' => $this->input->post('alamat'),
			'phone' => $this->input->post('phone'),
		...
		return redirect(base_url() . 'donatur_rutin/edit/' . $this->input->post('idDonatur'));
	}
	...
	{
		$data['donatur'] = $this->all_model->getDonaturRutinById((int) $id)->row();
		$this->load->view('donatur-rutin/view', $data);
	}
}